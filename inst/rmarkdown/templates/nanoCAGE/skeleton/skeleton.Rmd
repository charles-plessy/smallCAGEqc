...
### Hierarchical clustering of using Peason correlation

```{r hierarchical clustering, dev=c('png','svg')}
library(dendextend)
...
if (nbClusters > length(colorsDend)) stop ("Not enough colors (too many clusters)")
...
genesDendRev <- rev(as.hclust(genesDend)$height)
...
heatmapGenes <- NMF::aheatmap( corMat,
  ...
  Rowv = genesDend, Colv = genesDend )

# Dendrogram only: for visual comfort
(genesDendGg <- genesDend %>% as.ggdend %>% ggplot(horiz = F))
```
...
#     You can type 'genesDendGg' to display the dendrogram
