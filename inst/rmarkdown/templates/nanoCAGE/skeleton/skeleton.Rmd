...
colData(l1)$counts <- colSums(assay(l1))
colData(l1)$l1     <- colSums(assay(l1) > 0)
```
...
ranges2genes <- function(ranges, genes)
  findOverlaps(ranges, genes) %>%
    as(., "List") %>%
    extractList(mcols(genes)$gene_name, .) %>%
    unique %>%
    unstrsplit(";")

mcols(l1)$genes  <- ranges2genes(rowRanges(l1), gff)
genes <- rowsum(assay(l1), mcols(l1)$genes) %>% data.frame
```
...
ranges2annot <- function(ranges, annot, showClasses=NULL) {
  ...
  p <- findOverlapsBool(ranges, promoters(gff, 500, 500))
  e <- findOverlapsBool(ranges, gff[mcols(gff)$type == "exon",])
  t <- findOverlapsBool(ranges, gff[mcols(gff)$type == "transcript",])
...
colData(l1)[ranges2annot(showClasses=T)] <- rowsum(assay(l1), mcols(l1)$annotation) %>% t %>% DataFrame
```
...
```{r genesymbols_l1}
colData(l1)$genes <- colSums(genes > 0)
colData(l1)$geneSymbols <- countSymbols(genes) # Not the same!
```
...
plotAnnot(colData(l1), 'counts', LIBRARY)
```
