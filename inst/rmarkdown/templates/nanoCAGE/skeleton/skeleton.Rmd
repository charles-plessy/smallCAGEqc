...
# Promoter/Exon annotation made by hand.
# See https://gist.github.com/charles-plessy/9dbc8bc98fb773bf71b6 for hints
ANNOTATION   <- "/osc-fs_home/scratch/gmtu/annotation/homo_sapiens/gencode-23/gencode.v23.annotation.bed"
...
Metadata infered from the above:
...
mcols(ctss)$genes  <- findOverlaps(rowRanges(ctss), gff) %>%
           as(., "List") %>%
           extractList(mcols(gff)$gene_name, .) %>%
           unique %>%
           unstrsplit(";")

genes <- rowsum(assay(ctss), mcols(ctss)$genes)
```
...
rowRanges(l1)$promoter <- 
  findOverlapsBool(rowRanges(l1), promoters(gff, 500, 500))

rowRanges(l1)$exon <- 
  findOverlapsBool(rowRanges(l1), gff[mcols(gff)$type == "exon",])

rowRanges(l1)$transcript <- 
  findOverlapsBool(rowRanges(l1), gff[mcols(gff)$type == "transcript",])
...
rowRanges(l1)$annotation <- apply( mcols(l1)[,c("promoter", "exon", "transcript")]
                                 , 1
                                 , hannot2)

# Not idempotent !
colData(l1) %<>% cbind(DataFrame(t(rowsum(assay(l1),  mcols(l1)$annotation))[rownames(libs),]))
```
