...
# I prefer to have the coordinates as row names.
myCTSStagCount <- function(CAGEset) {
  ctss <- CTSStagCount(CAGEset)
  rownames(ctss) <- ctss %>% (function(X) do.call("sprintf", c(fmt="%s:%s%s", as.list(X))))
  ctss[,-1:-3]
}

l1 <- myCTSStagCount(myCAGEset)

l1Coords <-
  CTSScoordinates(myCAGEset) %>%
  (function (x)
    GRanges( x$chr
           , IRanges(x$pos, x$pos)
           , x$strand))
...
```{r annotation}
AnnotGRanges <- import.bed(ANNOTATION)
annot.l1 <- findOverlaps(l1Coords, AnnotGRanges) %>%
                as(., "List") %>%
                extractList(mcols(AnnotGRanges)$name, .) %>%
                unique %>%
                unstrsplit(",")
annot.l1 <- data.frame(feature=annot.l1) %>% hierarchAnnot
annot.l1 %>% summary
...
genes <- findOverlaps(l1Coords, gff) %>%
           as(., "List") %>%
           extractList(mcols(gff)$gene_name, .) %>%
           unique %>%
           unstrsplit(",")

genes <- rowsum(l1, genes)
```
